...
```
services/cloud/api/          config, main, hub, deps, mqtt_bridge, fusion_loop, routers/*
services/tools/replay/       the simulator that makes buses move
...
`services/cloud/api/test_module.py` runs with **no infrastructure at all** and that is
what keeps this property honest.

## Run it standalone

```bash
.venv/bin/uvicorn services.cloud.api.main:app --reload
